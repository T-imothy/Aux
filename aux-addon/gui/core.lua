...
function M.button(parent, text_height)
    ...
    set_frame_style(button, aux.color.button.background, aux.color.button.border)
    local highlight = button:CreateTexture(nil, 'HIGHLIGHT')
    highlight:SetAllPoints()
    highlight:SetTexture(aux.color.button.hover())
	...
	function mt.__index:update()
		for _, tab in self._tabs do
			if tab.group._selected == tab.id then
				...
				tab:Disable()
				tab:SetBackdropColor(aux.color.accent())
				tab.dock:Show()
				tab:SetHeight(29)
			else
